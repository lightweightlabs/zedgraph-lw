...
namespace ZedGraph
{
	...
	#region private Fields
      ...
      private float DrawLabel(Label label, CurveItem curve, Graphics g, PaneBase pane, float scaleFactor, GraphPane tmpPane, bool drawLegendSymbols, float x, float y)
      {
         float xEnd = 0;

         if (label._text != "" && label._isVisible)
         {
            ...
            FontSpec tmpFont = (label._fontSpec != null) ?
                     label._fontSpec : this.FontSpec;

            // This is required because, for long labels, the centering can affect the
            // position in GDI+.
            tmpFont.StringAlignment = StringAlignment.Near;
            xEnd = tmpFont.MeasureString(g, label._text, scaleFactor).Width + x + 2.5F * _tmpSize;
            if (drawLegendSymbols)
            {
               tmpFont.Draw(g, pane, label._text,
      ...
      public bool FindPoint(PointF mousePt, PaneBase pane, float scaleFactor, out int index)
      {
         ...
         if (_rect.Contains(mousePt))
         {
            ...
            if (i >= _numberOfColumns)
               i = _numberOfColumns - 1;

            int pos = i + j * _numberOfColumns;
            index = 0;
